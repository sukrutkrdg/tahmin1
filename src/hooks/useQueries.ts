...
const MARKET_ABI = ["function createPrediction(uint8 _asset, uint256 _threshold, uint8 _direction, uint8 _interval, uint8 _tokenType, uint256 _amount) external returns (uint256)"];
...
export function useCreatePrediction() {
  ...
  return useMutation({
    mutationFn: async (prediction: Prediction) => {
      ...
      if (!addresses || !addresses.predictionMarket) throw new Error("Kontrat adresi bulunamadı!");
...
export function useGetCallerUserProfile() { return useQuery({ queryKey: ["profile"], queryFn: async () => ({ name: "Kullanıcı", totalPoints: 0 }) }); }
